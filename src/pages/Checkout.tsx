import { Alert, Input, Radio, RadioGroup } from '@nextui-org/react'
import { AlertTriangle, ArrowLeft, ArrowRight, BanknoteIcon, Building2, CheckCircle2, Clock, CreditCard, Truck, User } from 'lucide-react'
      ...
      case 2:
        return (
          <div className='space-y-6'>
            ...
            >
              <Radio value='card' description='Pago seguro con tarjeta de crédito o débito' className='p-3 rounded-lg'>
              ...
              <Radio value='transfer' description='Transferencia bancaria directa' className='p-3 rounded-lg '>
                <div className='flex items-center gap-2'>
              ...
              <Radio value='deposit' description='Depósito en efectivo en banco o tienda de autoservicio' className='p-3 rounded-lg '>
                <div className='flex items-center gap-2'>
            ...
            {(paymentMethod === 'transfer' || paymentMethod === 'deposit') && (
              <Alert className='mb-4' variant='bordered' hideIcon startContent={<AlertTriangle className='w-5 h-5' />}>
                ...
                {bankAccounts.map((account, index) => (
                  <div key={index} className='bg-gray-50 p-4 rounded-lg'>
                    ...
                    <div className='space-y-1 text-sm'>
                      <p>
                        <span className='font-medium'>Beneficiario:</span> {account.holder}
                      </p>
                      <p>
                        <span className='font-medium'>Cuenta:</span> {account.accountNumber}
                      </p>
                      <p>
                        <span className='font-medium'>CLABE:</span> {account.clabe}
                      </p>
                    </div>
                  </div>
                ))}
              ...
              <div className='space-y-4'>
                {debitCards.map((card, index) => (
                  <div key={index} className='bg-gray-50 p-4 rounded-lg'>
                    ...
                    <div className='space-y-1 text-sm'>
                      <p>
                        <span className='font-medium'>Número de Tarjeta:</span> {card.cardNumber}
                      </p>
                    </div>
                  </div>
                ))}
        ...
        return (
          <div className='space-y-6'>
            ...
            <div className='bg-gray-50 p-4 rounded-lg'>
              ...
              <p>
                {formData.calle} {formData.numeroExt}
                {formData.numeroInt ? ` Int. ${formData.numeroInt}` : ''}
              </p>
            ...
            <div className='bg-gray-50 p-4 rounded-lg'>
              ...
              <p className='capitalize'>
                {paymentMethod === 'card' ? 'Tarjeta' : paymentMethod === 'transfer' ? 'Transferencia' : 'Depósito'}
              </p>
              {paymentMethod === 'card' && <p className='text-sm text-gray-600'>Tarjeta terminación {formData.numeroTarjeta.slice(-4)}</p>}
            </div>

            {(paymentMethod === 'transfer' || paymentMethod === 'deposit') && (
              <Alert className='mb-4' variant='bordered' hideIcon>
                <div className='space-y-2'>
      ...
      <div className='mb-8'>
        <div className='flex items-center justify-between relative'>
          ...
          {steps.map((step, index) => (
            <div key={step.title} className={`flex flex-col items-center ${index <= currentStep ? 'text-green-600' : 'text-gray-400'}`}>
              <div
