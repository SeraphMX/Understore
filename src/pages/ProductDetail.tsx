...
import { ArrowLeft, CheckCircle2, Clock, Heart, Image as ImageIcon, Send, Shield, Star, Truck } from 'lucide-react'
...
import { formatDate, formatPrice, formatUnit, getAvailableUnits, Unit } from '../types'
...
const ProductDetail = () => {
  ...
  const isFavorite = useSelector((state: RootState) => state.favorites.items.some((item) => item.id === id))
  ...
  if (!product) {
    return (
      <div className='text-center py-12'>
        ...
        <button onClick={() => navigate('/productos')} className='text-green-600 hover:text-green-700'>
          Volver a productos
        </button>
      </div>
    )
          ...
          <div className='p-8 md:w-1/2'>
            <div className='flex justify-between items-start'>
              ...
              <button
                ...
                className={`p-2 rounded-full transition-colors ${
                  isFavorite ? 'bg-red-50 text-red-500' : 'hover:bg-gray-100 text-gray-400 hover:text-red-500'
                }`}
              >
            ...
            <div className='flex items-center space-x-4 mb-6'>
              <div className='flex items-center border rounded'>
                <button onClick={() => setQuantity((q) => Math.max(1, q - 1))} className='px-4 py-2 hover:bg-gray-100'>
                  -
                </button>
                <input
                  type='number'
                ...
                />
                <button onClick={() => setQuantity((q) => Math.min(product.stock, q + 1))} className='px-4 py-2 hover:bg-gray-100'>
                  +
                </button>
              </div>
            ...
            <div className='border-t pt-6'>
              ...
              <span className='inline-block bg-gray-100 rounded-full px-3 py-1 text-sm'>{product.category}</span>
            </div>
          </div>
        </div>
          ...
          >
            ...
            <Tab
              ...
              title={
                <div className='flex items-center gap-2'>
                  <span>Reseñas</span>
                  <span className='bg-gray-100 text-gray-600 rounded-full px-2 py-0.5 text-xs'>{reviews.length}</span>
                </div>
              }
            >
              <div className='p-6'>
                <div className='flex justify-between items-center mb-6'>
                  ...
                  <button onClick={onOpen} className='bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition-colors'>
                    Escribir Reseña
                  </button>
                </div>
                ...
                <div className='space-y-6'>
                  ...
                  {reviews.length === 0 && (
                    <div className='text-center py-8 text-gray-500'>No hay reseñas aún. ¡Sé el primero en opinar!</div>
                  )}
                </div>
              </div>
            </Tab>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                  <div className='bg-gray-50 p-6 rounded-lg'>
                    ...
                    <div className='space-y-4 text-gray-600'>
                      <p>Todos nuestros productos están respaldados por nuestra garantía de satisfacción:</p>
                      <ul className='space-y-2'>
                        <li className='flex items-center gap-2'>
                          <CheckCircle2 className='w-5 h-5 text-green-600' />
                          <span>Garantizamos la fescura, sabor y calidad </span>
                        </li>
                        <li className='flex items-center gap-2'>
                          <CheckCircle2 className='w-5 h-5 text-green-600' />
                          <span>Reenvio gratuito en caso de problemas</span>
                        </li>
                        <li className='flex items-center gap-2'>
                          <CheckCircle2 className='w-5 h-5 text-green-600' />
                  ...
                  <div className='bg-gray-50 p-6 rounded-lg'>
                    ...
                    <div className='space-y-4'>
                      ...
                      <div className='flex items-start gap-3'>
                        ...
                        <div>
                          <p className='font-medium'>Envío discreto seguro</p>
                          <p className='text-gray-600'>Empaque especial para proteger los productos</p>
                        </div>
                      </div>
                      <div className='flex items-start gap-3'>
                        ...
                        <div>
                          ...
                          <p className='text-gray-600'>En compras mayores a $2,499</p>
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        <ModalContent>
          ...
          <ModalBody>
            <div className='space-y-4'>
              <div>
                ...
                <div className='flex gap-1'>
                  {[1, 2, 3, 4, 5].map((rating) => (
                    <button key={rating} onClick={() => setSelectedRating(rating)} className='focus:outline-none'>
                      <Star className={`h-6 w-6 ${rating <= selectedRating ? 'text-yellow-400 fill-current' : 'text-gray-300'}`} />
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div>
                <label className='block text-sm font-medium text-gray-700 mb-1'>Imágenes (opcional)</label>
          ...
          <ModalFooter>
            ...
            <Button color='primary' onPress={handleSubmitReview} startContent={<Send className='w-4 h-4' />}>
              Publicar Reseña
            </Button>
          </ModalFooter>
        </ModalContent>
