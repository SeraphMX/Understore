...
import { AlertTriangle, Bell, CheckCircle2, CreditCard, Package, Truck } from 'lucide-react'
    ...
    <Dropdown>
      <DropdownTrigger>
        <button className='hover:text-green-200'>
          <Badge content={unreadCount} color='danger' size='sm' className='text-tiny' isInvisible={unreadCount === 0}>
          ...
          <>
            {notifications.slice(0, 3).map((notification) => (
              <DropdownItem
                ...
                startContent={
                  <div className={`mt-1 ${getNotificationColor(notification.type)}`}>{getNotificationIcon(notification.type)}</div>
                }
            ...
            <DropdownItem className='border-t' onClick={() => navigate('/perfil?tab=notifications')}>
              <span className='w-full text-center text-sm text-blue-600'>Ver todas las notificaciones</span>
            </DropdownItem>
          </>
        )}
      </DropdownMenu>
