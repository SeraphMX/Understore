...
const Navbar: React.FC<NavbarProps> = ({ onCartClick }) => {
  ...
  return (
    <nav className="bg-green-600 text-white relative">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          <Link to="/" className="flex items-center space-x-2" onClick={closeMobileMenu}>
            <Sprout className="h-8 w-8" />
            <span className="text-xl font-bold">SemillasShop</span>
          </Link>
          ...
          <div className="hidden md:flex items-center space-x-6">
            {isAuthenticated && user?.isApproved ? (
              <>
                <Link to="/productos" className="hover:text-green-200">
                  Productos
                </Link>
                
                <Link to="/perfil?tab=favorites" className="hover:text-green-200">
                  <Badge
      ...
      />
      <div
        className={`absolute top-16 left-0 right-0 bg-green-600 z-50 transition-transform duration-300 ease-in-out transform ${
        ...
        <div className="container mx-auto px-4 py-4 space-y-4">
          {isAuthenticated && user?.isApproved ? (
            <>
              ...
              <Link
                to="/productos"
                className="block p-3 hover:bg-green-700 rounded-lg"
                onClick={closeMobileMenu}
              >
                Productos
              </Link>
              ...
              >
                <span>Carrito</span>
                {itemCount > 0 && (
                  <Badge color="danger" content={itemCount} />
                )}
              </button>
              ...
              >
                ...
                {favorites.length > 0 && (
                  <Badge color="danger" content={favorites.length} />
                )}
              </Link>
              ...
              <button
                onClick={handleLogout}
                className="w-full p-3 text-left text-red-200 hover:bg-green-700 rounded-lg"
              >
                Cerrar Sesión
              </button>
            </>
